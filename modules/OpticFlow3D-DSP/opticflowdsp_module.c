	...
	if(autopilot_mode == AP_MODE_OPTIC_FLOW)
	{
		...
		if (computervision_thread_has_results)
		{
			...
			if(threshold>300)
			{
				...
				cmd_euler.phi = vx + OFXInt;
				cmd_euler.theta = vy + OFYInt;
...
void *computervision_thread_main(void* data)
{
	...
	while (computer_vision_thread_command > 0)
	{
		...
		//	start_timer();
			...
			Tzp = *((int*)(visionTask.bufferReceive+2*sizeof(int)));
			erreur = msgFromDsp.dwArg2;

			sem_wait(&sem_results);
			sem_wait(&sem_optitrack);
			sem_wait(&sem_acc);
			//if(ins_impl.sonar_z<1000 || autopilot_mode != AP_MODE_ATTITUDE_Z_HOLD)	//For 2D Optic flow testing
			//Use optic flow when camera is above 0.1 meter from the ground
	#if USE_OPTITRACK_Z
			if(optitrack_z<100)
	#else
			if(ins_impl.sonar_z<100)
	#endif
			{
				Tx = 0;	 Ty = 0;
				vx = 0;	 vy = 0;
				Tz = 0;
				normVGPS = 0; normVOF = 0;
				start_timer_cmpl();
			}
			else
			{
				Tx = (float)(Txp)/100 - p_corr*(float)(Fx);
				Ty = (float)(Typ)/100 - q_corr*(float)(Fy);
				Tz = (float)(Tzp)/100;

				//normVGPS = sqrt(gps.ecef_vel.x*gps.ecef_vel.x + gps.ecef_vel.y*gps.ecef_vel.y + gps.ecef_vel.z*gps.ecef_vel.z);
				float Vx = FPS*optitrack_z*0.1*Tx/Fx;
				float Vy = FPS*optitrack_z*0.1*Ty/Fy;
				float Vz = FPS*optitrack_z*0.1*Tz;

				//Debug
				normVGPS = gps.ecef_vel.z;
				normVOF = Vz;
				/*
	#if USE_OPTITRACK_Z
	...
	*/
	...
	#if OPTITRACK_POS
			DOWNLINK_SEND_OPTICFLOW3D_DSP(DefaultChannel, DefaultDevice, &Tx, &Ty, &Tz, &erreur, &FPS, &gps.ecef_vel.x, &gps.ecef_vel.y, &gps.ecef_vel.z, &optitrack_z, &ins_impl.sonar_z, &normVGPS, &normVOF);
	#else
